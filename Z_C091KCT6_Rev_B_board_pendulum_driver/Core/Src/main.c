...
void ping_due(struct pendulum_t * p_pendulum, struct time_tracker_t * p_time_tracker);
void duly_ping(void);
...
int pwmSin[] = {  127,  110,  94,   78,   64,   50,
                37,   26,   17,   10,   4,    1,
                0,    1,    4,    10,   17,   26,
                37,   50,   64,   79,   95,   111,
                128,  144,  160,  176,  191,  205,
                218,  229,  238,  245,  251,  254,
                255,  254,  251,  245,  238,  229,
                218,  205,  191,  176,  160,  144 };
long count = 0;
...
int measure_offset(struct pendulum_t * p_pendulum) // remove power from motor, take magnetic readings
{
  ...
  const long electrical_pos  = elec_pos(pendulum_A.pos_array[pendulum_A.pos_array[10][0]][0]);
...
{
	AS5048A_update_pos_array();
	long electrical_pos  = elec_pos(pendulum_A.pos_array[pendulum_A.pos_array[10][0]][0]);
...
void update_max_min(struct pendulum_t * p_pendulum)
{
  ...
  p_pendulum->pos_reading_provisional_max = fmax(p_pendulum->pos_reading_provisional_max, dummy_var);
  p_pendulum->pos_reading_provisional_min = fmin(p_pendulum->pos_reading_provisional_min, dummy_var);
}
...
void ping_due(struct pendulum_t * p_pendulum, struct time_tracker_t * p_time_tracker)
{
	if (PENDULUM_NO != 0){return;}
	ping_count_current = (p_time_tracker->loop_time_current / p_pendulum->meta_period);
