...
void calculate_current_vector(struct pendulum_t * p_pendulum, long position, int magnitude)
{
  int dir;
  if (magnitude <= 0){dir = 36;}
  else dir = 12;
  ...
  if (abs(magnitude) > 100){magnitude = 100;} // don't care about the sign of magnitude any more.
  // TODO magnitude is a bad word for this job, replace with eg torque

  p_pendulum->current_vector[0] = (int) ( abs(magnitude) * pwmSin[dummyA] / 100.0 );
  p_pendulum->current_vector[1] = (int) ( abs(magnitude) * pwmSin[dummyB] / 100.0 );
  p_pendulum->current_vector[2] = (int) ( abs(magnitude) * pwmSin[dummyC] / 100.0 );
}
...
bool measure_due(struct time_tracker_t * time_track_var, long last_measure_time)
{
  ...
  if (dummy - last_measure_time >= time_track_var->measurement_interval) {return true;}
  else return false;
}
...
long pgzc(struct pendulum_t * p_pendulum) // returns the est. time at which the pendulum crosses zero going +ve, updates pendulum struct
{
  ...
  long positionA = p_pendulum->pos_array[(p_pendulum->pos_array[10][0]+9)%10][0]; // oldest position taken into account
