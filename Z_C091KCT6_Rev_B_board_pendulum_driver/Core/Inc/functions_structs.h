...
// location [10][1] is garbage, pos_array[][] was originally called my_array, there are still some legacy references to my_array
 ...
 long elec_pos(long sensor_reading);
  ...
  bool motor_update_due(struct time_tracker_t * time_track_var, long last_measure_time);
  ...
  void calculate_current_vector(struct pendulum_t * p_pendulum, long position, int magnitude); // current as in Amperes, not 'present' or dried grape
